...
(defn parse-map-line [line]
  (nth (re-find #"(\S+)" line) 1))

(defn tree-map-path-tree-count [tree-map [right-step down-step]]
...
(defn -main [& args]
  ...
  (let [input-file-name "./input.txt"
    ...
    (let [path-slopes [[1 1] [3 1] [5 1] [7 1] [1 2]]
          path-slopes-tree-counts (zipmap path-slopes
                                          (map #(tree-map-path-tree-count tree-map %)
                                               path-slopes))
          argmin-path-slope-tree-count (apply min-key #(get path-slopes-tree-counts %) path-slopes)
          path-slopes-tree-count-product (apply * (vals path-slopes-tree-counts))]
