...
(defn p1-is-row-valid?
  [{:keys [minc maxc letter password]}]
  (let [frequency (get (frequencies password) letter 0)]
    (and (>= frequency minc) (<= frequency maxc))))

(defn p2-parse-line
  [line]
  (let [[i1 i2 letter password] (rest (re-find #"(\d+)-(\d+)\s+(\w)\:\s(\w+)"
                                               line))]
...
(defn -main
  ...
  (let [input-file-name "./input.txt"
    ...
    (let [input-rows (map p1-parse-line input-lines)
          ...
          input-row-is-valid-count (count (filter identity
                                            input-row-is-valid-seq))]
